...
// In-memory data store for leads, quotes, demo bookings, service tickets
...
const mockServices: StoredService[] = [
  {
    id: 'S-3088',
...
// System instruction matching the user prompt strictly
const ADESH_SYSTEM_INSTRUCTION = `You are Adesh, the AI Sales & Technical Support Representative for Ganesh Enterprises, a trusted industrial supplier of heavy machinery, equipment, CNCs, and hardware tools.
...
const toolsConfig: FunctionDeclaration[] = [
  {
...
function handleToolExecution(name: string, args: Record<string, unknown>) {
...
// Fallback chat endpoint (using gemini-3.8-flash) for text testing or non-WebSocket fallbacks
  ...
  try {
    ...
    if (!ai) {
      return res.status(500).json({
        ...
        reply: "Thank you for calling Ganesh Enterprises. This is Adesh. Please make sure the GEMINI_API_KEY is configured in Settings > Secrets to activate live voice.",
      });
    }
    ...
    try {
      ...
      console.warn("gemini-3.8-flash busy, falling back to gemini-flash-latest:", modelErr);
      response = await ai.models.generateContent({
    ...
    let toolResultNotification: unknown = null;

    // Handle any function calls in unary mode
    ...
    res.json({
      reply: replyText || "Understood! I've noted that down for you. How else can I assist your team today?",
